    ...
    if(empty($name_err) && empty($address_err) && empty($salary_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                error_log("MySQLi Update Execute Error: " . mysqli_stmt_error($stmt));
                echo "Oops! Something went wrong. Please try again later. (Error Code: " . mysqli_stmt_errno($stmt) . ")";
            }
        } else {
            error_log("MySQLi Update Prepare Error: " . mysqli_error($link));
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                error_log("MySQLi Select (Update) Execute Error: " . mysqli_stmt_error($stmt));
                echo "Oops! Something went wrong. Please try again later. (Error Code: " . mysqli_stmt_errno($stmt) . ")";
            }
        } else {
            error_log("MySQLi Select (Update) Prepare Error: " . mysqli_error($link));
            echo "Oops! Database query preparation failed. Please try again later. (Error Code: " . mysqli_errno($link) . ")";
        }
